...
impl TuiNode for RecentTicketsMenu {
    fn measure(&self, proposal: LayoutProposal) -> LayoutSizeHint {
        let rows = if self.loading {
            1
        } else {
            self.visible_row_count().max(1).min(MAX_VISIBLE_ROWS.into())
        };
        let preferred_height = 1 + u16::try_from(rows).unwrap_or(MAX_VISIBLE_ROWS) * 2;
    ...
    fn render<'a>(&'a self, frame: &mut Frame, area: Rect, ctx: &mut RenderCtx<'a>) {
        ...
        if self.loading {
            let message = Text::from(Line::from(Span::styled(
                "Loading recent tickets…",
                Style::default().fg(tuicore::theme().muted_fg()),
            )));
            frame.render_widget(ratatui::widgets::Paragraph::new(message), self.list_area);
        } else {
